	...
	if(list != NULL && listPub != NULL && len > 0 && lenPub> 0)
		{
			for(j=0; j < lenPub ;j++)
			{
				if(list[i].idCliente==listPub[j].idCliente && listPub[j].isEmpty==0)
				{
...
int contarRubrosPorId(Publicacion* listPub,int lenPub, int i)
{
	...
	if( listPub != NULL && lenPub> 0)
	{
		for(j=0; j < lenPub ;j++)
		{
			if(listPub[i].NumeroDeRubro==listPub[j].NumeroDeRubro && listPub[j].isEmpty==0 )
			{
				contador++;
			}
		}
	...
	if(list != NULL && listPub != NULL && len > 0 && lenPub> 0)
		{
			for(j=0; j < lenPub ;j++)
			{
				if(list[i].idCliente==listPub[j].idCliente && listPub[j].isEmpty==0 && listPub[j].estado == estado )
				{
					//publicacion_mostrar(listPub, j);
					contador++;
				}
